...
export function applyEvent(state: LabState, e: SimulationEvent): LabState {
  ...
  switch (e.type) {
    ...
    case 'PAGE_FREE': {
      ...
      if (p) {
        if (state.firstLeafId === e.pageId) state.firstLeafId = p.next;
    ...
    case 'ROOT_CHANGE': {
      state.rootId = e.newRootId;
      state.height = e.height;
      const p = state.pages[e.newRootId];
      if (p) p.parentId = null;
      if (state.firstLeafId === null && p && p.type === 'leaf') state.firstLeafId = e.newRootId;
      m.rootChanges++;
      state.lastStructuralSeq = e.seq;
      break;
    }
    case 'LEAF_LINK': {
      ...
      p.prev = e.prev;
      p.next = e.next;
      if (e.prev === null && p.type === 'leaf') state.firstLeafId = e.pageId;
      break;
    }

    case 'DESCEND': {
      // 每次下降都从根页开始：看到根页的 DESCEND 就重置路径，
      // 否则批量插入会把成百上千次下降拼成一条无意义的长路径。
      if (e.pageId === state.rootId) {
        state.path = [e.pageId];
...
export function leafOrder(state: LabState): PageId[] {
  const out: PageId[] = [];
  let cur = state.firstLeafId;
