...
```
┌──────────────────────────────────────────────────────────────┐
│ UI 层        顶栏（引擎/视图/导出）+ 左右两条标签页侧栏 + 时间轴        │
│              左：操作 / 查询 / 参数 / 实验   右：状态 / 检查器 / 事件日志 │
├──────────────────────────────────────────────────────────────┤
...
侧栏由 `apps/web/src/components/ui/Rail.tsx` 提供：标签内容惰性渲染，
`available: false` 的标签直接不出现（能力裁剪），折叠状态与当前标签写进 `localStorage`。
`App.tsx` 只负责**分组**，不负责决定某个面板存不存在 —— 那仍然是能力的事。
...
**刷写与压实不在写路径上**——这是 LSM 写入快的全部原因，也是它最容易被误解的地方。
写路径只做两件事：把数据塞进 MemTable，满了就冻结并**排一个任务**，然后立刻返回。

```
put() ──► WAL 追加 ──► MemTable ──► 满？ ──► 冻结 + 排队 ──► 返回
                                              │
后台（命令间隙）──────────────────────────────┴──► 刷写 / 压实
```

仿真是确定性的，没有真线程，所以「后台」被建模成：
**每条命令结束时最多推进 `maxBackgroundJobs` 个任务**。这保留了本质
（活不在写路径上做、会积压），又不破坏可重放性。关掉 `backgroundCompaction`
就退回同步模式，两种模式跑同一组命令、最终数据必须完全一致（有测试守护）。

积压深度就是**压实债务**。债务还不上时写路径必须停下来还：

| 停顿原因 | 触发条件 | 写路径被迫做什么 |
|---|---|---|
| `immutable-full` | 已冻结的 MemTable 数 > `maxImmutableMemtables` | 同步跑一次刷写 |
| `l0-stop` | L0 文件数 ≥ `l0StopTrigger` | 同步跑一次压实（哪怕还没到软触发值 `l0CompactionTrigger`，停写阈值是硬底线） |

> 真实 RocksDB 在硬停顿之前还有一档**限速**（soft slowdown，按令牌桶压低写入速率）。
> 那需要墙钟才能表达，本仿真只建模硬停顿，已列入差异表。

**冻结队列里的数据是活的**：它还没落盘，但读路径必须能读到它，可视化里也必须看得见。
所以 `MEMTABLE_FREEZE` 事件携带全部条目，而不是只带一个条数 ——
只带条数会让积压期间的数据从界面上凭空消失。

## 3.10 WAL 的生命周期与崩溃恢复

WAL 不是个计数器，它真的存着数据，而且**只存还没落盘的那部分**：

```
写入   ──► 追加进「当前段」
冻结   ──► 当前段封口并绑定该 MemTable，新写入转入新段
落成 SST ──► 那个段被回收（WAL_TRUNCATE）
```

于是有一条不变式：**WAL 里保留的记录 = 崩溃后需要重放的记录**。
面板上的「WAL 待恢复」就是这个数，它随刷盘上下浮动，不会单调增长。

`crash` 命令把这条链路走完整：

1. `CRASH` —— MemTable、冻结队列、后台任务队列全在内存里，一起清空；SST 与 WAL 在「磁盘」上幸存；
2. `WAL_REPLAY` —— 按 lsn 顺序逐段重放，把记录装回 MemTable（重放期间**不再写日志**，否则日志会自我复制）；
3. 重放出来的 MemTable 立刻落成一个 SST，然后旧日志才敢丢弃（LevelDB 的恢复流程正是如此）；
4. `RECOVER_END`。
...
`apps/web/src/content/` 下有两份内容源，都是**纯数据**：

* `scenarios.ts` —— 全部引导实验（引擎 + 参数 + 命令序列）。刻意不依赖 store，
  这样它能在 node 环境里直接被测试导入；真正执行实验的 `runScenario` 拆在
  `run-scenario.ts`（它要碰 store，而 store 会起 Worker）。
* `engine-guide.ts` —— 五个引擎各一页 + 一页「怎么选」的讲解正文，
  按 `prose / list / diagram / table / callout` 五种块组织。

关键约束：**讲解里的每个机制都挂一个真实存在的实验**。
`guide.test.ts` 守护这条 —— 引用的 `scenarioId` 必须存在、且必须属于这一页讲的那个引擎，
否则点下去会跳到别的引擎。表格列数、图里不能出现未解析的内联标记，也一并断言。

正文只支持 `**加粗**` 与 `` `等宽` `` 两个内联标记，用二十行代码解析。
不引 markdown 库是刻意的：正文是自己写的、格式可控，引一个解析器只会给
「零依赖、可完全离线」这条约束多背一份包袱。
...
| LSM 后台并发 | 「每条命令推进 N 个任务」的确定性调度，无真线程、无墙钟 | 独立的 flush/compaction 线程池，按 CPU 与 IO 配额调度 |
| LSM 写限速 | 只建模硬停顿（写路径被迫同步干活） | 硬停顿之前还有一档按令牌桶的软限速 |
| WAL 持久性 | 只建模「先写日志 + 随刷盘回收 + 崩溃重放」，没有 fsync / 组提交 / 校验和 | fsync 策略、组提交、CRC 校验、日志复用 |
| 崩溃点 | 只能在命令边界崩溃 | 任意指令处崩溃，需要处理写了一半的记录 |
