...
| 1.6 | 所有关键状态变化都有对应 SimulationEvent | ✅ | 84 种事件；`replay.test.ts` / `secondary-index.test.ts` 断言重放 == 引擎快照 |
| 1.7 | 重操作在 Web Worker 中执行，主线程流畅 | ✅ | 单帧归约上限 400 事件；> 3000 事件的批次不自动播放 |
| 1.8 | 刷新页面后可从 IndexedDB 恢复实验会话 | ✅ | 持久化命令日志 + 确定性重放；e2e `会话持久化` |
| 1.9 | 核心插入与分裂逻辑有测试覆盖 | ✅ | 167 项单元测试（含随机负载下的多索引结构不变式校验；其中 89 项属于 Phase 2/3 的四个新引擎） |
...
| 3.16 | 块缓存 / SST 内部块结构 | ⏳ | 未做：LSM 引擎目前不接 Buffer Pool，SST 也不细分数据块/索引块 |

## Phase 3 续 —— 列存

引擎：`packages/simulation-core/src/engine/columnar-engine.ts`（`columnar`）

| # | 验收项 | 状态 | 说明 |
|---|---|---|---|
| C.1 | 行 → 列的转置：攒够一个行组才切成列块落盘 | ✅ | `ROW_GROUP_OPEN` / `COLUMN_CHUNK_WRITE` / `ROW_GROUP_SEAL`；写缓冲没满的行在查询前会先落盘 |
| C.2 | 每列按基数自动挑编码 | ✅ | delta（严格递增）/ dictionary（低基数）/ rle（连续重复）/ plain（压不动就回落），编码后不会比原样更大 |
| C.3 | **列裁剪**：只读谓词列 ∪ 投影列 | ✅ | `COLUMN_READ` 只对需要的列发；3D 里只有那几条竖列亮起来 |
| C.4 | **区间统计剪枝**：整块跳过 | ✅ | `ZONE_MAP_SKIP`，可在参数面板关掉做对照；剪枝只省 IO 不改答案（单测守护） |
| C.5 | 向量化执行 | ✅ | `VECTOR_BATCH`，批大小可调 |
| C.6 | IO 账单：列存读 vs 行存要读 | ✅ | 面板给出「省了百分之多少」；全跳过时账单同样成立（0 列 / 100%） |
| C.7 | 诚实呈现短板 | ✅ | 点查没有主键索引只能靠剪枝 + 逐列解码；更新/删除直接拒绝并说明「改一行要重写整个行组」 |
| C.8 | 「引擎快照 == 事件重放」一致性测试 | ✅ | `columnar.test.ts`：200 步混合负载 + 三组参数 + 确定性重放 |
| C.9 | 压缩算法本身（LZ4/ZSTD）、字典跨行组共享、后台合并 | ⏳ | 只建模了编码层，没有通用压缩；变更也还没有 merge tree |

## Phase 3 续 —— 哈希索引 KV

引擎：`packages/simulation-core/src/engine/kv-engine.ts`（`kv-hash`）

| # | 验收项 | 状态 | 说明 |
|---|---|---|---|
| K.1 | 追加写日志 + 内存哈希索引 | ✅ | `LOG_APPEND` 必定先于 `HASH_INDEX_SET`；活动文件写满就封口（`LOG_FILE_SEAL`），之后只读不写 |
| K.2 | 点查恒定一次寻址 | ✅ | `HASH_PROBE` 带冲突链步数；数据量翻 25 倍步数不变（单测守护）。键不存在时连磁盘都不碰 |
| K.3 | **范围扫描明确拒绝** | ✅ | 报错文案说清「哈希把键打散了」——这是两类 KV 的分水岭 |
| K.4 | 内存索引大小 = 规模上限 | ✅ | 面板显示索引字节数；反复更新同一批键不会撑大索引，只会撑大磁盘（单测守护） |
| K.5 | 覆盖写产生垃圾，合并回收 | ✅ | `MERGE_BEGIN` / `MERGE_END`；合并后索引重新指向新文件，点查照常命中 |
| K.6 | 桶数与冲突链的权衡 | ✅ | 桶少链长（单测守护），但复杂度量级不变 |
| K.7 | 与参考实现逐键比对 | ✅ | 400 步随机读写删后与普通 `Map` 完全一致 |
| K.8 | 「引擎快照 == 事件重放」一致性测试 | ✅ | `kv-hash.test.ts`：300 步混合负载 + 三组参数 + 确定性重放 |
| K.9 | 崩溃恢复（重放日志重建内存索引） | ⏳ | 未做。真实 Bitcask 靠 hint 文件加速重建，这条与 LSM 的 `crash` 可以合并做 |

## 跨引擎能力矩阵

| 能力 | InnoDB | PostgreSQL 堆表 | LSM | 列存 | 哈希 KV |
|---|---|---|---|---|---|
| `btree` | ✅ | ✅ | — | — | — |
| `clustered-index` | ✅ | — | — | — | — |
| `secondary-index` | ✅ | ✅ | — | — | — |
...
| `buffer-pool` | ✅ | ✅ | — | — | — |
| `lsm` / `compaction` / `bloom-filter` | — | — | ✅ | — | — |
| `wal` | — | — | ✅ | — | ✅ |
| `columnar` / `zone-map` / `vectorized` | — | — | — | ✅ | — |
| `kv` / `hash-index` | — | — | — | — | ✅ |
