...
| 1.6 | 所有关键状态变化都有对应 SimulationEvent | ✅ | 71 种事件；`replay.test.ts` / `secondary-index.test.ts` 断言重放 == 引擎快照 |
| 1.7 | 重操作在 Web Worker 中执行，主线程流畅 | ✅ | 单帧归约上限 400 事件；> 3000 事件的批次不自动播放 |
| 1.8 | 刷新页面后可从 IndexedDB 恢复实验会话 | ✅ | 持久化命令日志 + 确定性重放；e2e `会话持久化` |
| 1.9 | 核心插入与分裂逻辑有测试覆盖 | ✅ | 123 项单元测试（含随机负载下的多索引结构不变式校验；其中 55 项属于 Phase 2/3 的新引擎） |
...
| 3.11 | **刷写与压实不在写路径上**（后台任务队列） | ✅ | 写路径只冻结 + 排队就返回；`BG_JOB_SCHEDULED` / `BG_JOB_RUN`，积压深度即压实债务；可切回同步模式对照，两种模式最终数据必须一致 |
| 3.12 | **写停顿**：写入跑赢压实 | ✅ | `immutable-full` 与 `l0-stop` 两档硬停顿；`WRITE_STALL` + `forced` 任务。软限速需要墙钟，已列入差异表 |
| 3.13 | 冻结队列里的数据依然可读可见 | ✅ | `MEMTABLE_FREEZE` 携带全部条目；单测钉死「积压期间逻辑键数不能变少」 |
| 3.14 | **WAL 真的存数据**，并随刷盘逐段回收 | ✅ | 段的生命周期绑定一个 MemTable：冻结时封口、落成 SST 后 `WAL_TRUNCATE`。不变式：WAL 保留量 = 崩溃需重放量 |
| 3.15 | 崩溃 + 从 WAL 恢复 | ✅ | `crash` 命令清空全部内存结构后逐段重放，落成 SST 再丢弃旧日志；断言崩溃前后 `liveKeys()` 完全相等 |
| 3.16 | 列存（列块 + 压缩 + 向量化） | ⏳ | 未开始，仍属 Phase 3 范围 |
| 3.17 | 块缓存 / SST 内部块结构 | ⏳ | 未做：LSM 引擎目前不接 Buffer Pool，SST 也不细分数据块/索引块 |
