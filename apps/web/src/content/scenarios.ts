...
/**
 * 引导式实验的**唯一数据源**（纯数据，不依赖任何运行时）。
 *
 * 「引导实验」面板与「原理讲解」页都从这里读：讲解里每讲完一个机制，
 * 就能给出一个「跑这个实验」的按钮，直接把理论落到 3D 场景上。
 *
 * 真正执行实验的 `runScenario` 放在 run-scenario.ts —— 它要碰 store，
 * 而 store 会起 Worker；分开之后这份数据就能在 node 环境里直接跑测试。
 */
