...
当前实现：**Phase 0–4**，六种存储引擎可一键切换、并排对比：
...
六个引擎共用同一套事件协议、时间轴与 UI —— 面板与 3D 视图按**引擎能力**挂载，
所以「同一条 UPDATE 在六种数据库里各自发生了什么」可以直接切着看。
