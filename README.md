...
当前实现：**Phase 0–4**，七种存储引擎可一键切换、并排对比：
...
| **写时复制 B+ 树（LMDB）** | B+ 树，但旧版本永不改写 | 改一行**复制根到叶的整条路径**、提交只是**翻一下 meta 页**（没有 WAL、不需要恢复）、只读快照**零加锁**、叶子之间**没有链表**所以扫描要回溯、长读事务钉住旧页导致空间只涨不缩 |
| **Bε-树 / 分形树（TokuDB）** | B+ 树，但内部节点带消息缓冲 | 写**只碰根节点**、攒够一批才整体下推（写放大 = 树高 ÷ 批大小）、**盲写不用先读**、读要沿路翻每层缓冲、范围扫描前被迫把在途消息全推到叶子；缓冲容量调到 0 当场退化成 B+ 树 |

七个引擎共用同一套事件协议、时间轴与 UI —— 面板与 3D 视图按**引擎能力**挂载，
所以「同一条 UPDATE 在七种数据库里各自发生了什么」可以直接切着看。

> **LSM 与哈希 KV 都是 KV 存储，但走的是相反的路**：LSM 把数据在磁盘上排得有序，
> 换来范围扫描，代价是读要逐层探测；哈希彻底放弃顺序换来一次寻址，
> 代价是范围扫描根本做不了、键数被内存卡死。选型的全部内容就在这组对比里。
>
> **InnoDB 与写时复制 B+ 树共用同一副骨架，持久化哲学却完全相反**：一个就地改页、
> 靠 redo 日志兜底；另一个从不改写旧版本、提交只是换一个根。前者写得省、恢复要重放，
> 后者不需要恢复、但改一行要复制整条路径。
>
> **B+ 树与 LSM 不是两个阵营，是同一根轴上的两端**，轴的名字叫「写入要走多远才算数」。
> Bε-树站在中间：把消息缓冲容量调到 0，它当场退化成 B+ 树；调得越大越像 LSM。
> 那个参数就是可以直接拧的 ε。
...
| 4 | **写时复制 B+ 树**（复制路径 / meta 页双缓冲 / 只读快照 / 空闲表与页复用） | ✅ 已完成 |
| 4 | **Bε-树 / 分形树**（消息缓冲 / 攒批下推 / 盲写 / 读放大 / 扫描前强制结账） | ✅ 已完成 |
